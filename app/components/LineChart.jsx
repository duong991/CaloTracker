...
// Check if today's date matches the last date in the labels array
const checkDateMatch = (labels) => {
  const lastDate = labels[labels.length - 1]
  return formatDate(today) == lastDate
}

const LineChartComponent = observer(
  ({
    data = [70, 60.7, 82.5, 80, 70],
    labels = ["01/05", "03/05", "04/05", "05/05", "17/6"],
    color = "#ABCECF",
    dotColor = "#80A3A2",
  }) => {
    const {
      userInfoStore: { weight },
    } = useStores()
    if (data.length === 1) {
      data = [0, ...data]
      labels = ["", ...labels]
    ...
    if (!checkDateMatch(labels)) {
      data = [...data, weight]
      labels = [...labels, formatDate(today)]
    }
